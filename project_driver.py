...
if os.path.exists('b_mz.npy') and os.path.exists('bmz_errors.npy') and os.path.exists('kvalues.npy') and use_cache:
    ...
    np.save('./over_densityz7.22', perturbed_field.density)
...
initial_pos[:,1] *= 1.3 + 0.1 * np.random.randn(nwalkers)
initial_pos[:,2] *= 0.05 + 0.01 * np.random.randn(nwalkers)
initial_pos[:,3] *= 0.02 + 0.005 * np.random.randn(nwalkers)

sampler = emcee.EnsembleSampler(nwalkers, ndim, sa.log_post_bmz_b_errs, args=(kvalues, b_mz))
...
ax.errorbar(kvalues, b_mz, yerr = (sample[3]/(-np.exp((0.7*kvalues)+0.1)+2.2)) , linestyle = 'None',capsize=4, marker ='o') #yerr = bmz_errors*sample[2]
